...
def load_mcp_config():
    """Load MCP endpoint from .config.yaml or config.yaml"""
...
if __name__ == "__main__":
    ...
    # Get token from .config.yaml or fallback to environment variable
    try:
        endpoint_url = load_mcp_config()
        logger.info(f"Loaded MCP endpoint from .config.yaml: {endpoint_url}")
    except SystemExit:
        raise
    except Exception as e:
        logger.warning(
            f"Failed to load from .config.yaml: {e}, trying environment variable..."
        )
        endpoint_url = os.environ.get("MCP_ENDPOINT")
        if not endpoint_url:
            logger.error(
                "Please set the `MCP_ENDPOINT` environment variable or configure mcp_endpoint in .config.yaml"
            )
            sys.exit(1)
